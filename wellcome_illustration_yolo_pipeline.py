...
cross-checked by hand later:
  ...
  - --output-dir defaults to "wellcome_illustration_yolo_run" (siblings:
    ...
    "mdz_illustration_yolo_run"), so local state/temp files never collide
    either,
...
time as the other three illustration pipelines in their own tmux sessions
on the same RunPod pod - see "Running all four illustration pipelines at
once" below.
...
have a multi-GPU pod, pass --device cuda:1/cuda:2/cuda:3 to spread them out.
